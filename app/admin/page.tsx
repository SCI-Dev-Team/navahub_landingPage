...
export default function AdminPage() {
    ...
    try {
      if (selectedDocId) {
        ...
        if (createStep === 1) {
          ...
          setFeedback({
            success: true,
            message: "Step 1 (English) saved. Please complete Step 2 (Khmer).",
          });
          return;
        }

        if (!enDraftValues) {
          throw new Error("English data is missing. Please start from Step 1.");
        }

        const result = await createBilingualContentEntry({
          dataset,
          enValues: enDraftValues,
          kmValues: values,
        });
        setFeedback({
          success: true,
  ...
  return (
    <main className="min-h-screen bg-neutral-50 px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto mb-6 w-full max-w-7xl rounded-2xl border border-neutral-200 bg-white px-6 py-5 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#CC0000]">NavaHub Admin</p>
        <h1 className="mt-2 text-2xl font-bold text-neutral-900 sm:text-3xl">Content Management</h1>
        <p className="mt-1 text-sm text-neutral-600">Manage Firestore data with a consistent workflow for create, edit, and delete.</p>
      </div>
        ...
        <section className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <label className="space-y-1.5">
              ...
              <select
                className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2.5 text-sm text-neutral-800 outline-none ring-[#CC0000]/20 transition focus:border-[#CC0000] focus:ring-2"
                value={locale}
                onChange={(event) => {
              ...
              <div className="inline-flex rounded-xl border border-neutral-300 bg-white p-1">
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-sm font-medium transition ${
                    viewMode === "form" ? "bg-[#CC0000] text-white" : "text-neutral-700 hover:bg-neutral-100"
                  }`}
                >
                  Add New Data
                </button>
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-sm font-medium transition ${
                    viewMode === "list" ? "bg-[#CC0000] text-white" : "text-neutral-700 hover:bg-neutral-100"
                  }`}
                >
                  Data List
                </button>
          ...
          {/* Content Views */}
          <div>
            {/* Removed old single action button; replaced with explicit mode toggle */}
          </div>
          ...
          {viewMode === "list" ? (
                  ...
                  <div
                    ...
                    className={`rounded-xl border bg-white p-3 transition ${
                      selectedDocId === entry.docId
                        ? "border-[#CC0000]/60 shadow-[0_0_0_2px_rgba(204,0,0,0.08)]"
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <button
                        ...
                        className="text-left text-sm font-semibold text-neutral-900 transition hover:text-[#CC0000]"
                      >
                        {dataset === "upcomingEvents"
            ...
            <form className="mt-8 rounded-2xl border border-neutral-200 bg-white p-4 sm:p-5" onSubmit={handleSubmit}>
                ...
                <div className="mb-4 inline-flex rounded-xl border border-neutral-300 bg-white p-1">
                  <button
                    ...
                    className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${
                      createStep === 1 ? "bg-[#CC0000] text-white" : "text-neutral-700 hover:bg-neutral-100"
                    }`}
                  >
                    English
                  </button>
                  <button
                    ...
                    className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${
                      createStep === 2 ? "bg-[#CC0000] text-white" : "text-neutral-700 hover:bg-neutral-100"
                    }`}
                  >
                    Khmer
                  </button>
                  ...
                  <label key={field.key} className="space-y-1.5">
                    ...
                    {field.type === "boolean" ? (
                      <div className="flex h-[42px] items-center rounded-xl border border-neutral-300 bg-white px-3">
                        <input
                          type="checkbox"
                          className="h-5 w-5 rounded border border-neutral-300 accent-[#CC0000]"
                      ...
                      field.key === "description" ? (
                        <textarea
                          ...
                          rows={5}
                          className="w-full resize-y rounded-xl border border-neutral-300 bg-white px-3 py-2.5 text-sm text-neutral-800 outline-none ring-[#CC0000]/20 transition focus:border-[#CC0000] focus:ring-2"
                        />
                      ) : (
                        <input
              ...
              <button
                type="submit"
                disabled={submitting}
                className="mt-5 rounded-xl bg-[#CC0000] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#B00000] disabled:cursor-not-allowed disabled:opacity-60"
              >
                {submitting
                  ? "Saving..."
                  : selectedDocId
